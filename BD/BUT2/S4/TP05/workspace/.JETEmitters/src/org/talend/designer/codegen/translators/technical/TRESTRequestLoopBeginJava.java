...
public class TRESTRequestLoopBeginJava
{
  ...
  protected final String TEXT_10 = " =" + NL + "\t\t\t\t(java.util.Map<String, Object>) globalMap.get(\"restRequest\");" + NL + "" + NL + "\t\trestEndpoint = ";
  protected final String TEXT_11 = ";" + NL + "" + NL + "\t\tif (null == requestMessage_";
  protected final String TEXT_12 = ") {" + NL + "" + NL + "\t\t\tif (restTalendJobAlreadyStarted) {" + NL + "\t\t\t\tthrow new RuntimeException(\"request is not provided\");" + NL + "\t\t\t} else {" + NL + "\t\t\t\tif (!runInTalendEsbRuntimeContainer && null == thread4RestServiceProviderEndpoint) {" + NL + "\t\t\t\t\tString endpointUrl_";
  protected final String TEXT_13 = " = checkEndpointUrl(restEndpoint);" + NL + "\t\t\t\t\t// *** external thread for endpoint initialization" + NL + "\t\t\t\t\tthread4RestServiceProviderEndpoint = new Thread4RestServiceProviderEndpoint(this, endpointUrl_";
  protected final String TEXT_14 = ");" + NL + "\t\t\t\t\tthread4RestServiceProviderEndpoint.start();" + NL + "\t\t\t\t\t// *** external thread for endpoint initialization" + NL + "\t\t\t\t}" + NL + "" + NL + "\t\t\t\trestTalendJobAlreadyStarted = true;" + NL + "" + NL + "\t\t\t\tif(runInDaemonMode){" + NL + "\t\t\t\t\tThread.currentThread();" + NL + "\t\t\t\t\ttry {" + NL + "\t\t\t\t\t\twhile(true) {" + NL + "\t\t\t\t\t\t\tThread.sleep(60000);" + NL + "\t\t\t\t\t\t}" + NL + "\t\t\t\t\t} catch (InterruptedException e_";
  protected final String TEXT_15 = ") {" + NL + "\t\t\t\t\t\t// e_";
  protected final String TEXT_16 = ".printStackTrace();" + NL + "\t\t\t\t\t\t// throw new TalendException(e_";
  protected final String TEXT_17 = ", \"wholeJob\", globalMap);" + NL + "\t\t\t\t\t}" + NL + "\t\t\t\t}" + NL + "\t\t\t}" + NL + "\t\t\treturn;" + NL + "\t\t}";
