...
public class StaticMethodeEtu{
    ...
    public static ArrayDeque<Groupe> recupTableau0(AbstractChangementFactoryNP factory,String nomEtu){
        ArrayDeque<Groupe> arbre = new ArrayDeque<Groupe>();
        AbstractGroupeFactory factory2=factory.getGroupeFactory();
        Set<Etudiant> set = StaticMethodeEtu.getFactory().getGroupeFactory().getEtudiants(nomEtu);
        Iterator<Etudiant> iterator = set.iterator();
        Etudiant etu=null;
        if(iterator.hasNext()){
            etu=iterator.next();
        }
        Set<Groupe> setter =StaticMethodeEtu.getFactory().getGroupeFactory().getGroupesOfEtudiant(etu);
