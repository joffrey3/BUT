...
public class MaMemoire implements Memoire {
	...
	if(this.size() == this.capacity())
        ...
        if(this.contain(s)) throw new IllegalStateException(s + " est déjà présent en mémoire.");
        this.mem[this.index]=s;
        this.index++;
    }
    ...
    public String remove (){
        if (this.index == 0) throw new IllegalStateException();
        this.index--;
    ...
    public Boolean contain (String s){
	    ...
	    String m=mem[i];
	    if(m!=null){
            if(m.equals(new String(s.toString()))){
                res=true;
